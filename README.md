...
- A WhatsApp account that can scan an Evolution API QR Code when `/search-whatsapp-jobs` asks for connection.
...
`/setup` only creates the local candidate profile and email writing preferences. WhatsApp connection happens in `/search-whatsapp-jobs`: the command verifies `EVOLUTION_INSTANCE`, checks `connectionState`, and uses Evolution API's `instance/connect` QR Code flow when the phone is not linked yet.
...
- WhatsApp search depends on a valid Evolution API instance and a connected WhatsApp session; `/search-whatsapp-jobs` guides the QR Code connection flow when the session is missing.
